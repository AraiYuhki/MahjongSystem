...
{
    private static IEnumerable<(IHands, TestData)> BlessingOfHeavenTestDataSource()
    {
        var judge = new BlessingOfHeaven();
        ...
        yield return (judge, new TestData("子なので地和になってしまっている", false)
        {
            hands = new()
            {
                new TileData(TileType.WhiteDragon), new TileData(TileType.WhiteDragon), new TileData(TileType.WhiteDragon),
